...
func (s *S21State) OnEnter(event hsm.Event) hsm.State {
	...
	s.srv.Logger().Debug("->S21;")
	s.entered = true

	stateS211 := NewS211State(s.srv, s)
...
func (s *S21State) EventHandler(event hsm.Event) hsm.Transition {
	switch event.ID() {
	case eb.ID():
		return hsm.NewExternalTransition(event, NewS211State(s.srv, s), hsm.NopAction)
	case eh.ID():
		return hsm.NewExternalTransition(event, NewS21State(s.srv, s.parentState), hsm.NopAction)
	default:
